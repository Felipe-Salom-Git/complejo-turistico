...
export function CheckOutWidget() {
  ...
  // 2. Filter Maintenance where Checkout is Today (End Date was Yesterday)
  // Checkout = EndDate + 1. So we look for tickets ending yesterday.
  const yesterday = new Date(today);
  yesterday.setDate(yesterday.getDate() - 1);
  const yesterdayStr = yesterday.toLocaleDateString('es-AR', { year: 'numeric', month: '2-digit', day: '2-digit' }).split('/').reverse().join('-');

  const maintenanceEnds = tickets.filter(t => {
    if (!t.fechaFin) return false;
    const tDate = t.fechaFin.toString().split('T')[0];
    return tDate === yesterdayStr;
  });
